...
import { ChainId, Currency } from '@tiwatoyin/dyleum-sdk'
import { core } from '@pancakeswap/wagmi/chains'
...
export function getBlockExploreLink(
  ...
  if (!chain) return core.blockExplorers.default.url
  switch (type) {
    case 'transaction': {
...
export function getBlockExploreName(chainIdOverride?: number) {
  ...
  return chain?.blockExplorers?.default.name || core.blockExplorers.default.name
}

export function getBscScanLinkForNft(collectionAddress: string, tokenId: string): string {
  return `${core.blockExplorers.default.url}/token/${collectionAddress}?a=${tokenId}`
}
