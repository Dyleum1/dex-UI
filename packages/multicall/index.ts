...
export function createMulticall<TProvider extends Provider>(
  ...
  const multicall: MultiCall = async (abi: any[], calls: Call[], chainId = ChainId.CORE) => {
  ...
  const multicallv2: MultiCallV2 = async ({ abi, calls, chainId = ChainId.CORE, options, provider: _provider }) => {
  ...
  const multicallv3 = async ({ calls, chainId = ChainId.CORE, allowFailure, overrides }: MulticallV3Params) => {
    const multi = getMulticallContract(chainId, provider({ chainId }))
    if (!multi) throw new Error(`Multicall Provider missing for ${chainId}`)
