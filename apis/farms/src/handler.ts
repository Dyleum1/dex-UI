...
import { ChainId, CurrencyAmount, Pair } from '@tiwatoyin/dyleum-sdk'
...
const cakeBusdPairMap = {
  ...
  [ChainId.CORE]: {
    address: Pair.getAddress(TW[ChainId.CORE], BUSD[ChainId.CORE]),
    tokenA: TW[ChainId.CORE],
    tokenB: BUSD[ChainId.CORE],
  },
}

const getCakePrice = async (isTestnet: boolean) => {
  const pairConfig = cakeBusdPairMap[isTestnet ? ChainId.CMP : ChainId.CORE]
